      ...
      >
          <span className="relative inline-flex items-center">
              {['i', 'n', 'f', 'o', '{', '@', '}', 't', 'r', 'i', 's', 'h', 'r', 'a', 'm', 'o', 's', '.', 'c', 'o', 'm']
                  .map((char, index) => (
                  <span key={index} className={`duration-700 [transition-delay:${index * 0.02}s] group-hover:[transform:rotateY(360deg)] group-hover:text-zinc-300`}>
                      {char}
                  </span>
              ))}
          </span>
          <span className="icon-transition ml-2 3xl:ml-5 mt-3 md:mt-2 base:mt-10">
              {isCopied ? (
                  <BsCheck2 className="text-cyan-300 text-2xl md:text-[2.2rem] base:text-[2.5rem] xl:text-[8rem]" />
              ) : (
